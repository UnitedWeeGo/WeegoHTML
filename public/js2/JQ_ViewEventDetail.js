...
//					$this.find('.content').css('height',document.documentElement.clientHeight - resizeOffset - 48); // 43 is height of bottom bar (messages button)
				...
				function getSingleEvent() {
					var url = window.parent.domain + "/get.event.php";
					var params = {registeredId:window.parent.ruid, eventId:o.eventId};
					...
					$.get(url, params, function(data) {
						...
						if (o.event.getEventState() >= Event.state.decided && o.event.getEventState() <= Event.state.ended) {
							...
							setUpUI();
							window.parent.viewController.showEventDetail(o.event.eventId, false, o.event.showCountMeIn(), true);
						}
					});
				}
				...
				function handleGetSingleEvent(data) {
					...
					for (var i=0; i<allEventsXML.length; i++) {
						...
						o.event = window.parent.model.getEventById(id); // new Event();
					...
					*/
					window.parent.model.currentEvent = o.event;
					window.parent.model.getModelDataAsJSON();
				}
				
				function getRecentLocations() {
					var url = window.parent.domain + "/get.report.location.php";
					var params = {registeredId:window.parent.ruid, eventId:o.eventId};
					//if (o.event && o.event.lastUpdatedTimestamp) params.timestamp = o.event.lastUpdatedTimestamp;
					$.get(url, params, function(data) {
				...
				function handleGetRecentLocationsResponse(data) {
					...
					if ($(data).find('response').attr('code') == '291') {
						$(data).find('reportLocation').each(function() {
							...
							if (id != window.parent.model.loginParticipant.email) {
...
//					setScroll();
				...
				function enableFeedButton() {
					...
					$this.find(".feedButton").click(function() {
						window.parent.viewController.showMessages(o.event.eventId);
					});
				}
					...
					$this.find(".locationList").find(".locationCell").each(function() {
						...
						$(this).find(".locationInfo").click(function() {
							...
							if (id && loc.locationType == 'yelp') {
								window.parent.viewController.showYelpReview(loc);
							} else {
								window.parent.viewController.showAddLocations(id);
							}
						});
					});
				}
				...
				function enableMapButton() {
					...
					if (o.event.getEventState() >= Event.state.decided) {
						$this.find(".decidedMapCell").click(function() {
							window.parent.viewController.showAddLocations(o.event.getWinningLocation().locationId);
						});
					}
				}
				...
				function enableAddFriendsButton() {
					...
					$this.find('.participantList').find('.callToAction').click(function() {
						window.parent.viewController.showAddFriends();
					});
				}
				...
				function toggleVoteForLocationWithId(locationId) {
					...
					var url = window.parent.domain + "/xml.vote.php";
					var params = {registeredId:window.parent.ruid, xml:xmlStr};
...
//					setScroll();
				...
				function setInitialLocation() {
					window.parent.model.updateGeoLocation();			
				}
				
				function handleGeoLocationUpdated() {
					o.myLocation = window.parent.model.getGeoLocation();
...
//					$this.find('.actionSheet').append('<div class="button red removeEvent">Remove event</div>');
				...
				function handleCountMeInClick() {
					var url = window.parent.domain + "/mod.acceptevent.php";
					var params = {registeredId:window.parent.ruid, eventId:o.event.eventId, didAccept:"true"};
					$.post(url, params, function(data) {
				...
				function handleCountMeOutClick() {
					var url = window.parent.domain + "/mod.acceptevent.php";
					var params = {registeredId:window.parent.ruid, eventId:o.event.eventId, didAccept:"false"};
					$.post(url, params, function(data) {
						handleEventAcceptanceResponse(data);
					});
				}
